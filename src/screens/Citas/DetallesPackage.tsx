...
import type { BoxPackageV2, Product, ProductData } from "../../utils/Date.interface"
...
const DetallesPackage = () => {
  ...
  const selectPackage = () => {
    /*  setBoxPackage(data)
     navigation.goBack()
     navigation.goBack() */
    navigation.navigate("StoreSelectionView", data)
  }
  ...
  const hasStoreData = data.stores && data.stores.length > 0
  ...
  return (
    <Container
      className="flex-1"
      style={{ backgroundColor: colors ? (Platform.OS === 'web' ? colors.lightVibrant : colors.lightVibrant) : Colors.white }}
    >
      <View>
        {/* Header */}
...
export default DetallesPackage
