...
export default function GoalsSelect() {
  ...
  const data = route.params as { customerGoals: CustumerIntereses[] } | undefined;

  async function GetIntereses() {
    try {
      let array;
      const host = process.env.APP_BASE_API;
      const url = `/api/appchancea/customer-goals/${SesionToken}`;
      const header = await GetHeader(TokenAuthApi, "application/json");
      const response: CustumerIntereses[] = await HttpService("get", host, url, {}, header, setLoader);

      if (data) {
        setInteresesCustomerSelect(data.customerGoals);
        array = response.map((ele) => {
          const validate = data.customerGoals.some((e) => e.id === ele.id);
  ...
  async function Submit() {
    ...
    try {
      const host = process.env.APP_BASE_API;
      const url = `/api/appchancea/customers/${SesionToken}?page=0&size=100`;
      const header = await GetHeader(TokenAuthApi, "application/json");
      const response = await HttpService(
        "put",
        host,
        url,
        { ...user, customerGoals: InteresesCustomerSelect },
        header,
        setLoader
      );

      const updatedUser = { ...user, ...response, customerGoals: InteresesCustomerSelect };
      setUser(updatedUser);
      ...
      console.error("Error in GoalsSelect onSubmit:", err);
      if (err && err?.status) {
          ...
          >
            <View className="items-center mb-6 mt-4">
              <Text style={[font.Bold, { fontSize: 18, textAlign: "center", marginBottom: 15, color: Colors.black }]}>
                ¿Qué te interesa de otra persona?
              </Text>
              <Image
