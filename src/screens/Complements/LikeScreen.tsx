...
const LikeScreen = () => {
  ...
  async function GetMatch(pageNumber = 0, shouldAppend = false) {
    ...
    try {
      ...
      const url = `/api/appchancea/likes/${SesionToken}?customerSourceId.equals=${user?.id}&page=${pageNumber}&size=${PAGE_SIZE}`
      ...
      console.log(`/api/appchancea/likes/${SesionToken}?customerSourceId.equals=${user?.id}&page=${pageNumber}&size=${PAGE_SIZE}`)
  ...
  async function Consult(item: any) {
    try {
      ...
      const header = await GetHeader(TokenAuthApi, "application/json")
      const response = await HttpService("get", host, url, {}, header, setLoader)
      navigation.navigate("CustomerProfile", { Customer: response, type: "Like" })
    } catch (err: any) {
      console.error(JSON.stringify(err))
      if (err && err?.status) {
        ToastCall("error", "Error de conexión con el Servidor", "ES")
      ...
      <View style={{ flex: 1, alignItems: 'center' }}>
        {NoMatch.length > 0 || loading ? (
            ...
            renderItem={({ item, index }) => {
              if (item) {
                return (
                  <CardLayout
                    idUser={item.id}
                    photo={item.customerProfiles[0] ? { uri: item.customerProfiles[0].link } : undefined}
                    key={item.id}
          ...
          <View className=" w-[90%] max-w-[400px] h-1/2 items-center justify-center border rounded-2xl border-primary p-6 bg-white shadow-sm" style={{ alignSelf: 'center' }}>
            <FontAwesome6 name="heart" size={64} color={Colors.primary} style={{ marginBottom: 16 }} />
            <Text
              style={[
                font.Bold,
                {
            ...
            >
              Todavía no le has dado me cuadra a ningún chance.
            </Text>
          </View>
        )}
      </View>
