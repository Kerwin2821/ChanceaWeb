...
const STORAGE_KEY = 'CitasRecibidasS';


// Función para agregar o actualizar una cita (Create/Update)
export const setCitasRecibidasS = async (citas: Cita[]): Promise<void> => {
    try {
...
// Función para agregar o actualizar una cita (Create/Update)
export const saveCitasRecibidasS = async (cita: Cita): Promise<void> => {
  try {
...
export const getCitasRecibidasS = async (): Promise<Cita[]> => {
  try {
    const existingData = await AsyncStorage.getItem(STORAGE_KEY);
...
export const getCitasRecibidasSById = async (id: number): Promise<Cita | null> => {
  try {
    const existingData = await AsyncStorage.getItem(STORAGE_KEY);
    if (existingData) {
...
export const deleteCitasRecibidasS = async (id: number): Promise<void> => {
  try {
    const existingData = await AsyncStorage.getItem(STORAGE_KEY);
    if (existingData) {
      const parsedData: Cita[] = JSON.parse(existingData);
...
export const clearCitasRecibidasS = async (): Promise<void> => {
  try {
