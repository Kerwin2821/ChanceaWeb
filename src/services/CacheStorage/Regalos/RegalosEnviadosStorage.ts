...
const STORAGE_KEY = 'RegalosEnviadasS';


// Función para agregar o actualizar una Regalo (Create/Update)
export const setRegalosEnviadasS = async (Regalos: Regalo[]): Promise<void> => {
    try {
...
// Función para agregar o actualizar una Regalo (Create/Update)
export const saveRegalosEnviadasS = async (Regalo: Regalo): Promise<void> => {
  try {
...
export const getRegalosEnviadasS = async (): Promise<Regalo[]> => {
  try {
    const existingData = await AsyncStorage.getItem(STORAGE_KEY);
...
export const getRegalosEnviadasSById = async (id: number): Promise<Regalo | null> => {
  try {
    const existingData = await AsyncStorage.getItem(STORAGE_KEY);
    if (existingData) {
...
export const deleteRegalosEnviadasS = async (id: number): Promise<void> => {
  try {
    const existingData = await AsyncStorage.getItem(STORAGE_KEY);
    if (existingData) {
      const parsedData: Regalo[] = JSON.parse(existingData);
...
export const clearRegalosEnviadasS = async (): Promise<void> => {
  try {
